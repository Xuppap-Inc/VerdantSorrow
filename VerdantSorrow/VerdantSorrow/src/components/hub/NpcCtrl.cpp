...
void NpcCtrl::update()
{
	if (vt_->currTime() < dialogTimer)canTalk = false;
	else canTalk = true;
	if (canTalk) {
		auto& ihdlr = ih();
			...
			for (auto c : colliders) {
				if (c->isActive() && c->getEntity()->getComponent<PlayerCtrl>() != nullptr) {
					if (ihdlr.keyDownEvent() && ihdlr.isKeyDown(SDL_SCANCODE_E)) {
						...
						else if (dialogBox_->getComponent<DialogBoxMngr>()->isFinished() && dialogBox_->getComponent<DialogBoxMngr>()->isLastParagraph()) {
							canTalk = false;
